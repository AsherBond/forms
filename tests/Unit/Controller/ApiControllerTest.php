...
class ApiControllerTest extends TestCase {
	...
	public function testUpdateFormAllowsValidConfirmationEmailQuestionId(): void {
		...
		$this->confirmationEmailService->expects($this->once())
			->method('validateRecipientQuestionId')
			->with($form, 7);
	...
	public static function dataUpdateFormRejectsInvalidConfirmationEmailQuestionId(): array {
		return [
			...
			'question-from-another-form' => ['Question belongs to a different form'],
			'deleted-question' => ['Question has been deleted'],
		];
	}

	/**
	 * @dataProvider dataUpdateFormRejectsInvalidConfirmationEmailQuestionId
	 */
	public function testUpdateFormRejectsInvalidConfirmationEmailQuestionId(string $reason): void {
		$form = new Form();
		$form->setId(1);
		$form->setOwnerId('currentUser');
		...
		$this->confirmationEmailService->expects($this->once())
			->method('validateRecipientQuestionId')
			->with($form, 7)
			->willThrowException(new \InvalidArgumentException($reason));
